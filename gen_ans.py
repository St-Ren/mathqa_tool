...
def main():
	...
	split = 'dev'
	nlist = open(split + '.nlist').readlines()
	for i in range(len(nlist)):
		nlist[i] = json.loads(nlist[i])
	pr_f = open(split + '.pro','w')
	cor_f = open(split + '.ope','w')
	ac = 0.0
	count = 0.0
	...
	problems = [json.loads(s) for s in open(split + '.tok.json')]
	...
	for l in gen:
		...
		for hp in hypos:
			...
			if hyponum == ans:
				ac +=1
				cor_f.write(hp.strip()+'\n')
				p = problems[idx]['question'].replace('\n',' \t ')
				if s2n:
					p = sen2numre(p)
				pr_f.write(p + '\n')
				break
